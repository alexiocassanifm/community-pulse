...
import { validateContentType, validateZodBody } from "@/lib/api/speaker-helpers";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validation = await validateZodBody(request, anonymousFormSchema);
    if (validation.error) return validation.error;
    const data = validation.data;
    const deviceId = validation.rawBody.device_id;

    // Check for existing submission within 4h from same IP
    ...
    const insertData: AnonymousSubmissionInsert = {
      ...
      professional_role:
        data.professional_background?.professional_role === "other"
          ? data.professional_background?.professional_role_other || null
          : PROFESSIONAL_ROLES.find(
              (r) => r.value === data.professional_background?.professional_role
            )?.label || null,
      experience_level: data.professional_background?.experience_level || null,
      industry:
        data.professional_background?.industry === "other"
          ? data.professional_background?.industry_other || null
          : INDUSTRIES.find(
              (i) => i.value === data.professional_background?.industry
            )?.label || null,
