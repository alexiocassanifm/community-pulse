...
import { useAnonymousForm, DEFAULT_FORM_VALUES } from "./use-anonymous-form";
...
export function useMultiStepForm() {
  ...
  const goToNext = useCallback(async () => {
    ...
    // Validate current section fields before advancing
    const fieldsToValidate = Object.keys(
      form.getValues(currentSectionId as keyof AnonymousFormData) || {}
    ).map((field) => `${currentSectionId}.${field}` as const);
